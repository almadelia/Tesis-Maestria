\documentclass[letterpaper,12pt,doublespace]{article}
...
\author{\makebox[.9\textwidth]{Alma Maldonado Santiago\footnote{CIO. Le\'on, Guanajuato. M\'exico}}\\ \and Andres Christen\footnote{CIMAT. Guanajuato, M\'exico.}\\ \and Enrique Villa\footnote{CIMAT. Guanajuato, M\'exico.} \\ \and  Fabrizio Ruggeri\footnote{CRN IMATI, Milano, Italia.} }
\maketitle
...
\noindent En este trabajo se dise\~na una metodolog\'ia para la construcci\'on de un plan \'optimo de almacenamiento de transformadores, esta construcci\'on se realiza desde un enfoque bayesiano. Haciendo uso de una funci\'on de p\'erdida que ser\'a definida en un contexto, que permita minimizar los costos del almac\'en. Finalmente esta funci\'on ser\'a evaluada por simulaci\'on.}\\[0.1cm]

{\bf \noindent Palabras claves: Transformadores, confiabilidad, Estad\'istica Bayesiana, Funci\'on de utilidad, t-walk, optimizaci\'on, simulaci\'on.}\\[0.2cm]

{\begin{center}\bf Abstract\end{center}} 

\noindent In the 2006 the power transformers in subareas of Coatzacoalcos and Temascal (Veracruz), observed an unusual failure numbers, this fact led to the necessity of study the reliability's power transformers. The target was predict the failure numbers of their equipment per year. In this paper will design a methodology for the plan storage power transformers, around a bayesian point. Having use to a loss function will be define in a context that permit minimize  the storage cost. Finally this function will be proof for simulation.\\[0.1cm]


{\bf \noindent Keywords: Power transformers, Reliability, Bayesian Statistics, Loss function, t-walk, optimization , simulation.}\\[0.2cm]




\section{Modelaci\'on Bayesiana}
\begin{table}[h!]\small
\begin{center}
\caption { Tiempos de vida de transformadores de corriente, $t_i$ (meses), $e_i=1$ indica un dato censurado, $e_i=0$ es un dato no censurado.\label{uno}}
...
$(T,\underline{e})=\{t_i,e_i\}^n_{i=1}$, donde $e_i=1$ indica que 
el transformador $i$ aun sigui\'o en 
operaci\'on al final de 2006, lo que implica que  es un dato censurado por la derecha. Mientras que $e_i=0$, se refiere a un valor no censurado. La funci\'on de Verosimilitud (\cite{Meeker}) bajo 
este modelo se expresa 
de la siguiente manera:
\begin{eqnarray}\label{tiempos}
f(T|\underline{e},\beta,\eta)&=& \prod_{e_i=0}\frac{\beta}{\eta}\left(\frac{t_i}{\eta}\right)^{\beta-1}\exp \left\{-\left(\frac{t_i}{\eta}\right)^{\beta}\right\}\prod_{e_i=1}\exp\left\{-\left(\frac{t_i}{\eta}\right)^{\beta}\right\}.
\end{eqnarray}
...
\noindent La funci\'on de riesgo describe el desgaste de los transformadores a lo largo del tiempo. Al empezar su funcionamiento, el riesgo a fallar del transformador es peque\~no y conforme pasa el tiempo, debido al uso, el ambiente y otros factores, el riesgo aumenta paulatinamente 
...
El siguiente paso, es observar el comportamiento de la distribuci\'on predictiva a priori.  Al momento de asignar una distribuci\'on de probabilidad a los par\'ametros ($\beta$, $\eta$), estos no tienen interpretaci\'on cuantificable por experiencias. No es posible conocer expl\'icitamente sus valores por observaciones previas, solo tenemos conocimiento del fen\'omeno en general. Si se sabe la manera a priori del comportamiento del tiempo de vida de los transformadores entonces se tiene conocimiento de la distribuci\'on predictiva a priori. Esta distribuci\'on esta dada por:
 ...
 \noindent donde  $f(\beta,\eta)$ es la distribuci\'on conjunta a priori de $\beta$ y $\eta$ (\cite{lee} ).
\noindent La manera m\'as com\'un de establecer la distribuci\'on  a priori predictiva es empleando dos cuantiles. 

\begin{figure}
\begin{center}
\includegraphics[scale=1]{apriori1.pdf}
\end{center}
\vspace{-1.5 cm}\caption{Distribuci\'on predictiva a priori a modelar.}\label{dos}
\end{figure}
...
\noindent es la variable aleatoria a priori que modela el tiempo de vida de los transformadores. La Figura \ref{dos} muestra la distribuci\'on a priori predictiva, con los cuantiles fijados en los extremos. Sin embargo el centro de la distribuci\'on debe ser modelada. La informaci\'on de los cuantiles se representa por:
\begin{eqnarray*}
P(T\leq 80)&=&0.05\\
P(T\leq 480)&=&0.95
\end{eqnarray*}
...
\noindent Los par\'ametros encontrados describen el comportamiento a priori del tiempo de vida, no de los par\'ametros $\beta$  y $\eta$. Con ayuda de estos datos se pueden obtener los par\'ametros $a_1, b_1, a_2$ y $b_2$, notando lo siguiente:
...
\vspace{-0.5cm}\caption{{ Funci\'on de densidad a priori modelada con la informaci\'on disponible.}\label{risk1}}
\end{figure}
...
\vspace{-0.5cm}\caption{{Funci\'on de riesgo a priori modelada con la informaci\'on disponible.}\label{risk2}}
\end{figure}
...
\noindent La Ecuaci\'on (\ref{pos}) muestra que la distribuci\'on posterior de los par\'ametros, es una expresi\'on compleja y no f\'acil de evaluar.  En tales situaciones se recurre a la ayuda de m\'etodos computacionalmente intensivos que permiten simular una muestra de la distribuci\'on posterior, tales como algoritmos MCMC (\cite{Chen}), que son ampliamente
utilizados en estas situaciones.

\begin{figure}[h]
\begin{minipage}[b]{0.5\linewidth}
 \includegraphics[scale=0.2]{pos1.pdf}
  \caption{{\small Salida t-walk para $\beta.$}}
  \label{his1}
\end{minipage}
\begin{minipage}[b]{0.45\linewidth}
  \includegraphics[scale=0.2]{pos2.pdf}
  \caption{{\small Salida t-walk para $\eta.$}}
  \label{his2}
\end{minipage}
\end{figure}

\noindent Para conocer la muestra de la distribuci\'on posterior se uso el lenguaje de programaci\'on R, y la funci\'on t-walk que permite obtener tal muestra (\cite{R} ). Con la funci\'on t-walk se realizaron $900,000$ iteraciones para obtener la muestra deseada (\ref{pos}). Los resultados est\'an resumidos en las Figuras \ref{his1} y \ref{his2} que indican los valores en los que  se concentran $\beta$ y $\eta$. 
 
 \begin{figure}
\begin{center}
\includegraphics[scale=0.3]{logObj1.pdf} %logobj1
\end{center}
\vspace{-1cm} \caption{{Dentro de t-Walk, se suele emplear, la funci\'on logaritmo de la distribuci\'on objetivo para evaluar la convergencia del m\'etodo. La gr\'afica muestra el comportamiento de esta funci\'on.}\label{mlog}}
\end{figure}
...
\end{center}
\vspace{-1cm}\caption{{Series de convergencia de los par\'ametros $\beta$ y $\eta$, proporcionados por la funci\'on t-Walk. La serie 1 corresponde a $\beta$ y la serie 2 a $\eta$.}\label{serie}}
\end{figure}
...
\begin{table}[h!]\small
\centering
\caption{ Confiabilidad de los transformadores que segu\'ian funcionando despu\'es del 2006.}\label{conf}
\vspace{.2cm}
...
\end{center}
\vspace{-0.5 cm} \caption{{ Confiabilidad posterior empleando la muestra t-walk obtenida.}\label{CPos}}
\end{figure}
...
\end{center}
\vspace{-1 cm}{\caption{ Tasa de falla mensual, empleando la muestra de la funci\'on t-Walk.}\label{RPos}}
\end{figure}
...
\section{Estrategias de almacenamiento de transformadores}
...
\vspace{-2.8cm}\caption{Contrucci\'on del Plan de Almacenamiento.}\label{op}
\end{figure}
...
\vspace{-.3 cm}\caption{Parte de la muestra t-walk.}\label{ejee}
...
\end{eqnarray*} 
Lo que significa que la p\'erdida es la suma de las \'areas delimitadas por debajo de cero, multiplicada por $c$, donde $c$ se tiene que establecer con informaci\'on adicional. Sin perder generalidad se puede suponer $c=1$.
...
\end{center}
\vspace{-1.5 cm} \caption{Almacenamiento de transformadores, hasta $10$ unidades de tiempo, mostrando la p\'erdida sobre ese per\'iodo.}\label{tr}
\end{figure}
...
\end{center}
\vspace{-1 cm} \caption{ P\'erdidas Esperadas con la Pol\'itica A, usando $t=480$, $\delta=6,8,12$ y Differentes Valores de $n_0$.}\label{d6s}
\end{figure}
...
\begin{table}[h!]\small
\centering
\caption{ Pol\'itica A: Valores de las P\'erdidas Esperadas con $\delta=6$.}\label{c1}
...
\begin{table}[h!]\small
\centering
\caption{Pol\'itica A: Valores de las P\'erdidas Esperadas con $\delta=8$.}\label{c33333}
\vspace{0.3 cm}\begin{tabular}{ccccccccc}
\toprule[0.6mm]
$\bf{n_0}$&\bf{1} &                   \bf{2} &                   \bf{3} &                   \bf{ 4 }&                    \bf{ 5}&              \bf{ 6} &               \bf{ 7} & \bf{8} \\
\hline
...
\begin{table}[h!]\small
\centering
\caption{Pol\'itica A: Valores de las P\'erdidas Esperadas con $\delta=12$.}\label{c777}
...
\begin{table}[h!]\small
\begin{center}
\caption{ Resultados de la Pol\'itica A.}\label{taba1}
...
\end{table}
\noindent En la Tabla \ref{taba1} se muestran los valores \'optimos de transformadores iniciales que deben tenerse en el almac\'en, para un periodo de tiempo de $40$ a\~nos con diferentes valores de $\delta$, bajo la pol\'itica A. Es normal observar que entre mayor sea el periodo de espera de un transformador solicitado, mayor ser\'a el n\'umero de transformadores requeridos para el almacenamiento. Los costos asociados son cero, indica que a partir de la cantidad $n_0$ de transformadores, no habr\'a p\'erdidas en cuanto a la falta de ellos.
...
\end{center}
\vspace{-1 cm}\caption{ P\'erdidas Esperadas con la Pol\'itica B, usando $\delta=6$ y r=0.1}\label{u01p6}
\end{figure}

\begin{table}[h!]\small
\centering
\caption{ Pol\'itica B: Valores de las P\'erdida  Esperadas con $\delta=6$, $r=0.1$.}\label{r11}
...
\vspace{-1 cm}\caption{ P\'erdidas Esperadas con la Pol\'itica B, usando $\delta=8$ y r=0.1}\label{fi}
\end{figure}
\begin{table}[h!]\small
\centering
\caption{ Pol\'itica B: Valores de las P\'erdidas Esperadas con $\delta=8$, $r=0.1$.}\label{r12}
\begin{tabular}{ccccccccc}
\toprule[0.6mm]
$\bf{n_0}$ &\bf{1} &                   \bf{2} &                   \bf{3} &                   \bf{ 4 }&                    \bf{ 5}&              \bf{ 6} &               \bf{ 7} & \bf{8} \\
\hline
...
\noindent Finalmente para la pol\'itica C, al observar los resultados de la Tabla \ref{resuFF} los costos aumentan significativamente respecto a la pol\'itica A y B. Ahora se consideran costos por unidad de tiempo, tanto de falta de ellos como de almacenamiento. As\'i que se busca no  tener transformadores en el almac\'en pero tampoco necesitarlos, raz\'on por la cual, los costos de optimizaci\'on para esta pol\'itica son altos y los valores de $n_0$ asociados son menores a los de las pol\'iticas anteriores.
\begin{table}[h!]\small
\begin{center}
...
\noindent A lo largo de este paper hemos planteado una manera de proponer una funci\'on, que cuantifique  la perdida esperada por la falta de transformadores en el almac\'en. Existen distintas formas de plantear el remplazamiento de los transformadores, inclusive otras funciones de p\'erdida podr\'ian ser exploradas. Sin embargo dentro del desarrollo de este trabajo se propuso una posible soluci\'on y se presentaron los resultados.

%\begin{thebibliography}{99}
%
%\bibitem{riesgo} Peter M. Lee (2004). \textbf{``Bayesian 
%Statistics an introduction''}. A member of the Hodder Headline Group LONDON . \label{PL}
%
%\bibitem{riesgo} William Q. Meeker and Luis A. Escobar (1998). \textbf{``Statistical Metohds for Reliability Data''}. Wiley series in Probability and Mathematical Statistic . \label{me}
%
%\bibitem{mi} Ming-Hui Chen, Qi-Man Shao and Joseph G. Ibrahim(2000). \textbf{``Monte Carlo Methods in Bayesian Computation''}.
%Springer-Verlag .\label{mi}
%
%
...
%\end{thebibliography}
\bibliographystyle{chicagoa}
\bibliography{nueva}        % qhe.bib is the name of our database
